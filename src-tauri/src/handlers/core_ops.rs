use oxide_core::{
    AgentType, CudaDiscoveryResult, HarnessEngine, HarnessEvidence, InferenceRequestMetadata,
    LocalSkill, PrivacyGuard, SkillsEngine, SlicedContext, SwarmDag, SymbolChunk, TaskNode,
    TokenixEngine, VerifierType, WaveScheduler, WorktreeManager, discover_cuda,
};
...
#[tauri::command]
pub fn slice_differential_context(
    source: String,
    target_symbol: String,
) -> Result<SlicedContext, String> {
...
pub fn create_task_worktree(
    ...
    let target = manager
        .create_task_worktree(&task_id, &branch_name)
        .map_err(|e| e.to_string())?;
    Ok(target.to_string_lossy().into_owned())
}

#[tauri::command]
pub fn validate_swarm_dag_tasks(
    task_descriptions: Vec<(String, String)>,
...
#[tauri::command]
pub async fn run_local_harness_check(
...
pub fn match_diagnostic_skills(diagnostic: String) -> Vec<LocalSkill> {
    ...
    engine
        .match_skills_for_diagnostic(&diagnostic)
        .into_iter()
        .cloned()
        .collect()
}
...
#[tauri::command]
pub fn sanitize_prompt_for_remote(
    prompt: String,
    worker_id: String,
    project_id: String,
) -> (String, InferenceRequestMetadata) {
...
#[tauri::command]
pub fn discover_forged_tools(
    workspace_path: String,
) -> Result<Vec<oxide_core::ForgedToolSummary>, String> {
    let engine = oxide_core::ForgeEngine::new(PathBuf::from(workspace_path));
    engine.discover_tools().map_err(|e| e.to_string())
}

#[tauri::command]
pub fn synthesize_forged_tool(
    workspace_path: String,
    req: oxide_core::ForgeSynthesisRequest,
) -> Result<oxide_core::ForgeSynthesisResult, String> {
    let engine = oxide_core::ForgeEngine::new(PathBuf::from(workspace_path));
...
#[tauri::command]
pub fn parse_claude_conventions(
    workspace_path: String,
) -> Result<Vec<oxide_core::ClaudeConvention>, String> {
    let bridge = oxide_core::ClaudeBridge::new(PathBuf::from(workspace_path));
    bridge.parse_claude_md_files().map_err(|e| e.to_string())
}

#[tauri::command]
pub fn discover_claude_slash_commands(
    workspace_path: String,
) -> Result<Vec<oxide_core::ClaudeSlashCommand>, String> {
    let bridge = oxide_core::ClaudeBridge::new(PathBuf::from(workspace_path));
    bridge.discover_claude_commands().map_err(|e| e.to_string())
}

#[tauri::command]
pub fn execute_ox_read(
    workspace_path: String,
    file_path: String,
    offset: Option<usize>,
    limit: Option<usize>,
) -> Result<oxide_core::OxReadResult, String> {
    let arsenal = oxide_core::ClaudeArsenal::new(PathBuf::from(workspace_path));
    arsenal
        .ox_read(&file_path, offset, limit)
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub fn execute_ox_edit(
    workspace_path: String,
    file_path: String,
    old_str: String,
    new_str: String,
) -> Result<oxide_core::OxEditResult, String> {
    let arsenal = oxide_core::ClaudeArsenal::new(PathBuf::from(workspace_path));
    arsenal
        .ox_edit(&file_path, &old_str, &new_str)
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub fn execute_ox_write(
    workspace_path: String,
    file_path: String,
    content: String,
) -> Result<(), String> {
    let arsenal = oxide_core::ClaudeArsenal::new(PathBuf::from(workspace_path));
    arsenal
        .ox_write(&file_path, &content)
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub fn execute_ox_grep(
    workspace_path: String,
    pattern: String,
    ext_filter: Option<String>,
) -> Result<oxide_core::OxGrepResult, String> {
    let arsenal = oxide_core::ClaudeArsenal::new(PathBuf::from(workspace_path));
    arsenal
        .ox_grep(&pattern, ext_filter.as_deref())
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub fn execute_ox_bash(
    workspace_path: String,
    command: String,
) -> Result<oxide_core::OxBashResult, String> {
    let arsenal = oxide_core::ClaudeArsenal::new(PathBuf::from(workspace_path));
    arsenal.ox_bash(&command).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn get_cortex_device_capabilities() -> oxide_core::DeviceCapabilities {
    oxide_core::CortexHal::resolve_device()
}

#[tauri::command]
pub fn generate_cortex_embedding(text: String) -> Vec<f32> {
    let engine = oxide_core::Qwen3EmbeddingEngine::new();
    engine.embed_text(&text)
}

#[tauri::command]
pub fn search_cortex_knowledge_graph(
    chunks: Vec<(String, Option<String>, String)>,
    query: String,
    top_k: usize,
) -> Result<Vec<oxide_core::CortexQueryResult>, String> {
    let mut index = oxide_core::SurrealMTreeIndex::new(oxide_core::MTreeIndexConfig::default());
    for (file_path, symbol_name, content) in chunks {
        index.insert_chunk(&file_path, symbol_name, &content);
    }
    index.search_knn(&query, top_k).map_err(|e| e.to_string())
}
