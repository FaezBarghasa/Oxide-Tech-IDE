...
### 4. Global Search & Replace Engine (`Ctrl+Shift+F`)
- **Ripgrep-Powered Streaming Search**: Blazing fast multithreaded regex search across project workspaces with real-time JSON streaming to UI.
- **Symbol & File Mask Filtering**: Filter results by glob masks (e.g. `*.rs`, `src/**`) and search Rust AST symbols (structs, traits, enums, functions).
- **Batch Inline Replacements**: Find and replace occurrences within files with regex capturing group support.

### 5. Cargo Test Explorer & Visual Conflict Resolution
...
### 6. Step-by-Step MCU & Hardware Debugger (`Shift+F9`)
...
### 7. Full-Stack Visual Engineering Workstation
...
| **Global Search** | `ripgrep` (Multithreaded JSON stream parser) |
...
| Action | Primary Shortcut | Secondary / Alternate |
| :--- | :--- | :--- |
| **Search Everywhere** | `Shift+Shift` (Double Shift) | `Ctrl+P` |
| **Find in Files (Global Search)** | `Ctrl+Shift+F` (`Cmd+Shift+F`) | `Ctrl+F` |
